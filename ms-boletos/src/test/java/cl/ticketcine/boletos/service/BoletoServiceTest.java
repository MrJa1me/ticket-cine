...
import cl.ticketcine.common.exception.DuplicateResourceException;
 ...
 * repositorio, mapper y productor de Peliculas Kafka.
 */
...
class BoletoServiceTest {
    ...
    // Mock del productor de Peliculas para evitar publicar mensajes reales en Kafka.
    @Mock
    ...
    void setUp() {
        ...
        lenient().when(boletoMapper.toResponse(any(Boleto.class))).thenAnswer(invocation -> {
            ...
            if (boleto.getPelicula() != null) {
                response.setIdPelicula(boleto.getPelicula().getIdPelicula());
            }
        ...
        lenient().doAnswer(invocation -> {
            ...
            if (request != null && boleto != null) {
                ...
                ProyPelicula Pelicula = new ProyPelicula();
                Pelicula.setIdPelicula(request.getIdPelicula());
    ...
    private Boleto crearBoletoSimulado(Integer id) {
        ProyPelicula Pelicula = ProyPelicula.builder()
                .idPelicula(faker.number().numberBetween(1, 10))
                .nombrePelicula(faker.rockBand().name())
                .build();
     ...
     */
    @Test
    void create_DeberiaCrearBoletoYEnviarPelicula_CuandoRequestEsValidoYCodigoEsUnico() {
     ...
     */
    @Test
    void update_DeberiaActualizarBoletoYEnviarPelicula_CuandoElCodigoNoCambia() {
        Integer id = 5;
     ...
     */
    @Test
    void update_DeberiaActualizarBoletoYEnviarPelicula_CuandoElCodigoCambiaYNoEstaDuplicado() {
        Integer id = 5;
        Boleto boletoExistente = crearBoletoSimulado(id);
        BoletoRequest request = crearBoletoRequestSimulado();
