...
jar tf ms-catalogo\target\cl-ticketcine-catalogo-0.0.1-SNAPSHOT.jar | findstr /C:"PeliculaMapperImpl.class" >nul 2>&1
if %ERRORLEVEL% NEQ 0 (
    echo ERROR: El JAR no contiene PeliculaMapperImpl. Revisa la compilacion Maven/MapStruct.
    pause
    exit /b 1
)
...
echo   Gateway    : http://localhost:9000/api/v1/Peliculas
